...
This PGXN package is version `0.1.6`.
...
`1.1.0`, and `1.2.0` through the public `0.1.x` line up to `0.1.6`:
...
install script. `bench-check` enforces absolute benchmark thresholds and
`bench-guard` compares repeated-run medians/p90 values against the checked-in
baseline under `.bench/`. `pgxn-package` builds a release tarball under
`release/`.
