...
describe('benchmark: recursive_closure_stress_ms', () => {
  let db: PgLiquidTestDb;

  beforeAll(async () => {
    db = await createIsolatedPgLiquidDb('pg_liquid_bench_recursive_stress');
  });
  ...
  it('evaluates deeper transitive closure workloads', async () => {
    ...
    const seedProgram = joinProgram([
      generateManagementChainAssertions(chainNStress, predicate),
    ...
    expect(result.value).toBe(expectedChainClosureCount(chainNStress));
