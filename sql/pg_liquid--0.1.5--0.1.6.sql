...
strict
security definer
set search_path = pg_catalog, liquid
parallel restricted;

create or replace function liquid.compound_roles(compound_type text)
returns text[]
language sql
stable
security definer
...
create or replace function liquid.validate_taxonomy(subclass_predicate text)
returns table(
  issue_type text,
  subject_literal text,
  related_literal text,
  detail text
)
language sql
stable
...
set search_path = pg_catalog, liquid
as $$
  with subclass_edges as (
    select child.literal as child_literal,
           parent.literal as parent_literal
    from liquid.edges edge
    join liquid.vertices predicate_v
      on predicate_v.id = edge.predicate_id
    join liquid.vertices child
      on child.id = edge.subject_id
    join liquid.vertices parent
      on parent.id = edge.object_id
    where edge.is_deleted = false
      and predicate_v.literal = $1
  ),
  taxonomy_nodes as (
    select child_literal as class_literal
    from subclass_edges
    union
    select parent_literal
    from subclass_edges
  ),
  self_parent_issues as (
    select 'self_parent'::text as issue_type,
           child_literal as subject_literal,
           parent_literal as related_literal,
           format('class %s cannot be a subclass of itself', child_literal) as detail
    from subclass_edges
    where child_literal = parent_literal
  ),
  cycle_walk as (
    with recursive walk(origin_literal, current_literal, path) as (
      select child_literal,
             parent_literal,
             array[child_literal, parent_literal]::text[]
      from subclass_edges
      where child_literal <> parent_literal
      union all
      select walk.origin_literal,
             edge.parent_literal,
             walk.path || edge.parent_literal
      from walk
      join subclass_edges edge
        on edge.child_literal = walk.current_literal
      where not edge.parent_literal = any(walk.path)
    )
    select distinct
           'cycle'::text as issue_type,
           walk.origin_literal as subject_literal,
           walk.current_literal as related_literal,
           format('cycle detected from %s back through %s', walk.origin_literal, walk.current_literal) as detail
    from walk
    join subclass_edges edge
      on edge.child_literal = walk.current_literal
     and edge.parent_literal = walk.origin_literal
  ),
  missing_reference_issues as (
    select 'missing_referenced_class'::text as issue_type,
           node.class_literal as subject_literal,
           null::text as related_literal,
           format(
             'class %s only appears in taxonomy edges and has no additional defining facts',
             node.class_literal
           ) as detail
    ...
    where not exists (
      select 1
      from liquid.edges edge
      join liquid.vertices subject_v
        on subject_v.id = edge.subject_id
      join liquid.vertices predicate_v
        on predicate_v.id = edge.predicate_id
      where edge.is_deleted = false
        and subject_v.literal = node.class_literal
        and predicate_v.literal <> $1
    )
  )
  select issue_type, subject_literal, related_literal, detail
  from self_parent_issues
  union all
  select issue_type, subject_literal, related_literal, detail
  from cycle_walk
  union all
  select issue_type, subject_literal, related_literal, detail
  from missing_reference_issues
  order by 1, 2, 3 nulls first;
$$;

create or replace function liquid.validate_instances(instance_predicate text, subclass_predicate text)
returns table(
  issue_type text,
  subject_literal text,
  related_literal text,
...
set search_path = pg_catalog, liquid
as $$
  with instance_edges as (
    select instance_v.literal as instance_literal,
           class_v.literal as class_literal
    from liquid.edges edge
    join liquid.vertices predicate_v
      on predicate_v.id = edge.predicate_id
    join liquid.vertices instance_v
      on instance_v.id = edge.subject_id
    join liquid.vertices class_v
      on class_v.id = edge.object_id
    where edge.is_deleted = false
      and predicate_v.literal = $1
  ),
  taxonomy_nodes as (
    select child_v.literal as class_literal
    from liquid.edges edge
    join liquid.vertices predicate_v
      on predicate_v.id = edge.predicate_id
    join liquid.vertices child_v
      on child_v.id = edge.subject_id
    where edge.is_deleted = false
      and predicate_v.literal = $2
    union
    select parent_v.literal as class_literal
    from liquid.edges edge
    join liquid.vertices predicate_v
      on predicate_v.id = edge.predicate_id
    join liquid.vertices parent_v
      on parent_v.id = edge.object_id
    where edge.is_deleted = false
      and predicate_v.literal = $2
  ),
  invalid_taxonomy_classes as (
    select distinct subject_literal as class_literal
    from liquid.validate_taxonomy($2)
    where issue_type in ('self_parent', 'cycle')
    union
    select distinct related_literal as class_literal
    from liquid.validate_taxonomy($2)
    where issue_type = 'cycle'
      and related_literal is not null
  ),
  dangling_class_issues as (
    select 'dangling_class_reference'::text as issue_type,
           instance_literal as subject_literal,
           class_literal as related_literal,
           format(
             'instance %s references class %s, but that class is absent from taxonomy predicate %s',
             instance_literal,
             class_literal,
             $2
           ) as detail
    from instance_edges
    where class_literal not in (select class_literal from taxonomy_nodes)
  ),
  invalid_class_issues as (
    select 'invalid_class_reference'::text as issue_type,
           instance_literal as subject_literal,
           class_literal as related_literal,
           format(
             'instance %s references class %s, which fails taxonomy validation for %s',
             instance_literal,
             class_literal,
             $2
           ) as detail
    from instance_edges
    where class_literal in (select class_literal from invalid_taxonomy_classes)
  )
  select issue_type, subject_literal, related_literal, detail
  from dangling_class_issues
  union all
  select issue_type, subject_literal, related_literal, detail
  from invalid_class_issues
  order by 1, 2, 3;
$$;
...
begin
  ...
  expected_roles := liquid.compound_roles(compound_type);
...
begin
  ...
  expected_roles := liquid.compound_roles(p_compound_type);
  if coalesce(array_length(expected_roles, 1), 0) = 0 then
...
drop function if exists liquid._compound_roles(text);

revoke all on function liquid.query_as(text, text) from public;
revoke all on function liquid.read_as(text, text) from public;
revoke all on function liquid._ensure_edge(text, text, text) from public;
revoke all on function liquid._tombstone_edge(text, text, text) from public;
revoke all on function liquid._build_source_row_key(jsonb, text[]) from public;
revoke all on function liquid._build_role_values(jsonb, jsonb) from public;
revoke all on function liquid._project_row_normalizer_bindings(bigint, jsonb) from public;
revoke all on function liquid._deproject_normalizer(bigint) from public;
revoke all on function liquid._apply_row_normalizer_change(bigint, jsonb, jsonb) from public;
revoke all on function liquid.tg_apply_row_normalizer() from public;
revoke all on function liquid.create_row_normalizer(regclass, text, text, jsonb, boolean) from public;
revoke all on function liquid.drop_row_normalizer(regclass, text, boolean) from public;
revoke all on function liquid.rebuild_row_normalizer(regclass, text) from public;
